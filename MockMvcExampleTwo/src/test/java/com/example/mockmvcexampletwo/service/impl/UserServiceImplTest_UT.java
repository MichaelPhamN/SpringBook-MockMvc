...
public class UserServiceImplTest_UT {
    @InjectMocks
    ...
    public void findUsersByEmail() throws SQLException {
        ...
        when(userDao.findUsersByEmail(email)).thenReturn(users);
        int expectedResponse = 10;
        assertEquals(expectedResponse, userService.findUsersByEmail(email).size());
    }
