...
import com.example.mockmvcexampletwo.dao.UserDao;
...
public class UserDaoImpl implements UserDao {
    ...
    public int addUser(User user) throws SQLException {
        if(user == null) {
            throw new InvalidDataRequestException("User object is not null");
        }
    ...
    public User findUserById(Integer id) throws SQLException {
        if(id == null) {
            throw new InvalidDataRequestException("User id is not null");
        }
        User user = null;
        try {
    ...
    public List<User> findUsersByEmail(String email) throws SQLException {
        ...
        try {
            Connection conn = config.getConnection();
            String sql = "SELECT acc.id, acc.email, acc.password, acc.isAdmin FROM Account as acc WHERE acc.email like ?";
            PreparedStatement preparedStatement = conn.prepareStatement(sql);
